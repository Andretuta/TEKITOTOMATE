...
const { log, readJson, writeJson, delay, delayWithJitter, isConnectionError, trackGroupFailure, trackGroupSuccess, shuffleArray } = require('./utils');
const { getSpeedConfig, getSpeedMode, PARALLEL_CONFIG, WHATSAPP_GROUPS_DB, TELEGRAM_CHATS_DB, BASE_DIR, MEDIA_CACHE_DIR, checkRateLimit, registerBroadcast } = require('./config');
...
async function sendInBatches(items, sendFunction, config, platform) {
    const results = { success: 0, failed: 0, errors: [], connectionDead: false };
        ...
        const promises = batch.map(async (itemId) => {
            ...
            for (let attempt = 1; attempt <= retries; attempt++) {
                try {
                    ...
                    // Se for erro de conexão, não fazer retry — abortar
...
async function syncGroups() {
    ...
    try {
        ...
        const result = { found: groupIds.length, added, total: currentGroups.length };
...
async function sendToAll(message, imageUrl = null, directMedia = null, useTwitter = false) {
    ...
    // Nota: o aviso visual do rate limit já é feito pelo queue.js antes de chamar esta função.
    // Aqui apenas registramos o broadcast para o histórico.
    ...
    if (!media && imageUrl) {
        ...
        try {
            ...
            media = { buffer: buf, mimetype: media.mimetype, mediaPath: media.mediaPath };
    ...
    if (_isConnected() && _sock && wppGroups.length > 0) {
        // Verificar se o socket está realmente vivo
        if (!isSocketAlive(_sock)) {
            log('⚠️ Socket WebSocket não está vivo! Pulando envios WhatsApp.');
            wppResults = { success: 0, failed: wppGroups.length, errors: [{ id: 'all', error: 'Socket morto' }], connectionDead: true };
        } else {
            const profile = getSpeedConfig();
            ...
            wppResults = await sendInBatches(
                ...
                async (groupId) => {
                    if (media && media.buffer) {
                        ...
                        if (isVideo) {
                            await _sock.sendMessage(groupId, { video: media.buffer, caption: message || '' });
                        } else if (isDocument) {
                            await _sock.sendMessage(groupId, { document: media.buffer, caption: message || '', mimetype: media.mimetype });
                        } else {
                            await _sock.sendMessage(groupId, { image: media.buffer, caption: message || '' });
                        }
                    } else {
                        await _sock.sendMessage(groupId, { text: message || '📣 Nova mensagem!' });
                    }
                },
                profile.whatsapp,
                'WhatsApp'
        ...
        tgResults = await sendInBatches(
            ...
            async (chatId) => {
                if (media && media.buffer) {
                    const isVideo = media.mimetype?.includes('video');
                    if (isVideo) {
                        await _telegramBot.sendVideo(chatId, media.buffer, { caption: message || '' });
                    } else {
                        await _telegramBot.sendPhoto(chatId, media.buffer, { caption: message || '' });
                    }
                } else if (imageUrl) {
                    await _telegramBot.sendPhoto(chatId, imageUrl, { caption: message || '' });
                } else {
