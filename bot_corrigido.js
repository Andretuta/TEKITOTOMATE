...
// 🔧 CORREÇÃO: Só remove grupo quando o BOT sai/é removido
wpp.on('group_leave', async (notif) => {
    try {
        // Verificar se o bot ainda está no grupo
        const chat = await wpp.getChatById(notif.chatId);
        
        // Se não conseguir acessar o chat, significa que o bot foi removido
        if (!chat) {
            const updated = readJson(WHATSAPP_GROUPS_DB).filter(id => id !== notif.chatId);
            writeJson(WHATSAPP_GROUPS_DB, updated);
            log('🔴 Bot foi removido do grupo WhatsApp:', notif.chatId);
        } else {
            // Bot ainda está no grupo, apenas alguém saiu
            log('👋 Alguém saiu do grupo (bot permanece):', notif.chatId);
        }
    } catch (error) {
        // Se der erro ao acessar, provavelmente o bot foi removido
        const updated = readJson(WHATSAPP_GROUPS_DB).filter(id => id !== notif.chatId);
        writeJson(WHATSAPP_GROUPS_DB, updated);
        log('🔴 Bot foi removido do grupo WhatsApp (erro ao acessar):', notif.chatId);
    }
});
